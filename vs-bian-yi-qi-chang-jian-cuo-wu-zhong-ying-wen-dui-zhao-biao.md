...
VC6.0编译时出现这样的错误：
Configuration: max - Win32 Debug--Compiling...test.cppLinking...LINK : warning LNK4076: invalid incremental status file "Debug/test.ilk"; linking nonincrementally
原因：
VC编译时可以选择incremental linking（渐增式编译），也就是每次重新编译并不编译所有的源文件，只编译改动过的文件。而编译器怎么知道哪些编译过哪些每编译过呢，除了检查修改时间外，这个ilk文件也是很重要的信息。如果你因为误操作或者VC自己的BUG引起了ilk文件的损坏，下一次编译时当然无法根据ilk进行incremental linking了。但是在这种情况下也没什么大问题，VC自己会解决的，所以也只出了一个警告。
解决办法：
